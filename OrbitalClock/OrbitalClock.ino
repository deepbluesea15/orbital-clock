/* ============================================================================
   ORBITAL CLOCK  v1.0
   A fourteen-face display for the Waveshare 1.51" transparent OLED
   driven by an ESP32-S3 SuperMini.

   https://github.com/deepbluesea15/orbital-clock

   FACES (tap BOOT to cycle; the current one is remembered across power cycles)
      1 NEON       big glitching digits over a drifting starfield
      2 DIGITAL    plain clock with seconds, readable across the room
      3 RAIN       falling glyph columns, time punched through the middle
      4 DECRYPT    digits scramble and lock, framed by live chip telemetry
      5 WEATHER    current conditions, animated icon (Open-Meteo, no API key)
      6 FORECAST   five-day strip
      7 MOON       phase disc with illumination, age, next full and new
      8 ORRERY     six planets at their real heliocentric longitudes
      9 ORBIT      orbital dial, hour/minute/second bodies round a ringed sun
     10 TERMINAL   scrolling ship's log with a live clock
     11 FLYOVER    wireframe terrain scrolling past under a crescent moon
     12 TESSERACT  rotating 4D hypercube, projected 4D->3D->2D
     13 JELLYFISH  drifting jellies with pulsing bells and bubbles
     14 BOUNCE     the screensaver, still chasing that corner
   ...
   BUILD  Board: "ESP32S3 Dev Module", USB CDC On Boot: Enabled
...
static void cfgLoad() {
  ...
  uint8_t f   = prefs.getUChar("face", 0);
  prefs.end();
  // Guard the index: a firmware update can change how many faces exist, and a
  // stale value from an older build would run straight off the switch.
  if (f < FACE_COUNT) face = f;
}

// NVS is flash and wears out, so this never writes from the render loop.
// A change arms a timer; mashing the button through every face still costs
// one write, a few seconds after you stop.
static uint8_t  faceStored  = 255;
static uint32_t faceFlushAt = 0;

static void facePersistTick() {
  if (!faceFlushAt || millis() < faceFlushAt) return;
  faceFlushAt = 0;
  if (face == faceStored) return;
  prefs.begin("orbital", false);
  prefs.putUChar("face", face);
  prefs.end();
  faceStored = face;
}
...
static void handleRoot() {
  ...
  h += F("<!doctype html><html><head><meta charset=utf-8>"
         ...
         "</style></head><body>"
         "<h1>ORBITAL SETUP</h1><div class=s>One-time configuration &middot; " FW_VERSION "</div>"
...
static void faceDecrypt() {
  ...
  // Live chip telemetry across the top. temperatureRead() jitters by a degree
  // or two between samples, so hold each reading for a second instead of
  // flickering at 25fps.
  static float coreC = 0;
  static uint32_t coreAt = 0;
  if (coreAt == 0 || now - coreAt > 1000) { coreAt = now; coreC = temperatureRead(); }

  char buf[26];
  u8g2.setFont(u8g2_font_4x6_tr);
  snprintf(buf, sizeof(buf), "CORE %.0fC", coreC);
  dStr(0, 7, buf);
  ...
  // more telemetry along the bottom: uptime, clock speed, link quality
  uint32_t up = (millis() - bootMillis) / 1000;
  u8g2.setFont(u8g2_font_4x6_tr);
  snprintf(buf, sizeof(buf), "UP %02lu:%02lu:%02lu",
           (unsigned long)(up / 3600UL),
           (unsigned long)((up / 60UL) % 60UL),
           (unsigned long)(up % 60UL));
  dStr(0, 62, buf);

  snprintf(buf, sizeof(buf), "%uMHZ", (unsigned)getCpuFrequencyMhz());
  dStr((W - strW(buf)) / 2, 62, buf);

  if (WiFi.status() == WL_CONNECTED) snprintf(buf, sizeof(buf), "%ddBm", (int)WiFi.RSSI());
  else                               snprintf(buf, sizeof(buf), "NO LINK");
  dStrRight(W, 62, buf);
}
...
// ------------------------- FACE: MOON ---------------------------------------
// Phase from the Sun-Moon elongation using the main periodic terms of each
// body's motion. A flat "days since a known new moon" model is simpler but
// drifts up to 7% illumination either way, enough to name the wrong phase
// near a quarter; this holds inside 1.5% and needs nothing from the network.
static const double SYNODIC_M = 29.530588853;

static double moonElong() {                    // degrees, 0 = new, 180 = full
  double d = ((double)time(nullptr) - 946728000.0) / 86400.0;   // since J2000
  double Ls = 280.460 + 0.9856474 * d;
  double Ms = radians(fmod(357.528 + 0.9856003 * d, 360.0));
  double lamS = Ls + 1.915 * sin(Ms) + 0.020 * sin(2 * Ms);
  double Lm = 218.316 + 13.176396 * d;
  double Mm = radians(fmod(134.963 + 13.064993 * d, 360.0));
  double lamM = Lm + 6.289 * sin(Mm);
  double D = fmod(lamM - lamS, 360.0);
  if (D < 0) D += 360.0;
  return D;
}
...
static void faceMoon() {
  char buf[26];
  double D   = moonElong();
  double th  = radians(D);
  double age = D / 360.0 * SYNODIC_M;
  float  lit = (float)((1.0 - cos(th)) * 0.5);

  u8g2.setFont(u8g2_font_4x6_tr);
  dStr(0, 7, "MOON");
  snprintf(buf, sizeof(buf), "%.0f%% LIT", lit * 100.0f);
  dStrRight(W, 7, buf);
  dDashed(0, 10, W);

  // Outline the whole disc, then fill only the sunlit part. The terminator is
  // an ellipse, so per scanline the boundary sits at halfwidth * cos(D).
  const int cx = 29, cy = 37, R = 19;
  dCircle(cx, cy, R);
  for (int y = -R + 1; y <= R - 1; y++) {
    float hw = sqrtf((float)(R * R - y * y));
    float tx = hw * (float)cos(th);
    int xa, xb;
    if (D < 180.0) { xa = (int)ceilf(tx); xb = (int)hw; }            // waxing: lit right
    else           { xa = -(int)hw;       xb = (int)floorf(-tx); }   // waning: lit left
    if (xb >= xa) dHLine(cx + xa, cy + y, xb - xa + 1);
  }

  const int px = 56;
  u8g2.setFont(u8g2_font_4x6_tr);
  dStr(px, 21, moonName(age));

  double perDay = 360.0 / SYNODIC_M;
  double toFull = fmod(180.0 - D + 360.0, 360.0) / perDay;
  double toNew  = fmod(360.0 - D, 360.0) / perDay;

  u8g2.setFont(u8g2_font_5x7_tf);
  snprintf(buf, sizeof(buf), "AGE %.1fd", age);      dStr(px, 34, buf);
  snprintf(buf, sizeof(buf), "FULL %.1fd", toFull);  dStr(px, 45, buf);
  snprintf(buf, sizeof(buf), "NEW  %.1fd", toNew);   dStr(px, 56, buf);
}

// ------------------------- FACE: ORRERY -------------------------------------
// Real heliocentric positions: each planet's J2000 mean longitude plus one
// turn per orbital period. Mean elements only, no eccentricity, which runs a
// couple of degrees off the truth and is indistinguishable at this size.
// Orbits are drawn as an inclined view so the ellipses use the panel's width.
struct Planet {
  const char *name; float period; float l0; float peri; float ecc;
  uint8_t rx; uint8_t rad;
};
static const Planet PLAN[] = {
  { "MERCURY",    87.969f, 252.25084f,  77.45645f, 0.205630f, 12, 1 },
  { "VENUS",     224.701f, 181.97973f, 131.60246f, 0.006773f, 19, 1 },
  { "EARTH",     365.256f, 100.46435f, 102.94719f, 0.016709f, 27, 2 },
  { "MARS",      686.980f, 355.45332f, 336.04084f, 0.093405f, 35, 1 },
  { "JUPITER",  4332.589f,  34.40438f,  14.72847f, 0.048498f, 46, 3 },
  { "SATURN",  10759.220f,  49.94432f,  92.59887f, 0.055546f, 58, 2 },
};

// Mean longitude alone treats every orbit as a circle, which puts Mercury 24
// degrees from where it actually is and Mars 11. The equation of center is
// three sine terms and brings the whole set inside a degree.
static float planetLon(const Planet &pl, double d) {
  double L = pl.l0 + 360.0 * d / (double)pl.period;
  double M = radians(fmod(L - pl.peri, 360.0));
  double e = pl.ecc;
  double C = (2 * e - e * e * e / 4) * sin(M)
           + 1.25 * e * e * sin(2 * M)
           + (13.0 / 12.0) * e * e * e * sin(3 * M);
  double lon = fmod(L + degrees(C), 360.0);
  if (lon < 0) lon += 360.0;
  return (float)lon;
}
static const int PLAN_N = sizeof(PLAN) / sizeof(PLAN[0]);
static const float ORR_TILT = 0.36f;         // vertical squash; keeps Saturn clear of the footer

static void faceOrrery() {
  char buf[30];
  const int cx = 64, cy = 34;

  double days = ((double)time(nullptr) - 946728000.0) / 86400.0;   // since J2000
  int sel = (int)((millis() / 2600) % PLAN_N);                     // cycling callout

  // dotted orbit rings -- engraved look, and far less lit area than solid
  for (int i = 0; i < PLAN_N; i++) {
    float rx = PLAN[i].rx, ry = PLAN[i].rx * ORR_TILT;
    for (int a = 0; a < 360; a += 12) {
      float r = a * 0.01745329f;
      dPixel(cx + (int)(cosf(r) * rx), cy + (int)(sinf(r) * ry));
    }
  }
  ...
  for (int i = 0; i < 8; i++) {
    float a = i * 0.785398f;
    dPixel(cx + (int)(cosf(a) * 5), cy + (int)(sinf(a) * 5 * ORR_TILT));
  }

  for (int i = 0; i < PLAN_N; i++) {
    float r = planetLon(PLAN[i], days) * 0.01745329f;
    int x = cx + (int)(cosf(r) * PLAN[i].rx);
    int y = cy + (int)(sinf(r) * PLAN[i].rx * ORR_TILT);
    dDisc(x, y, PLAN[i].rad);
    if (i == sel) dCircle(x, y, PLAN[i].rad + 3);   // tie the dot to the label
  }
  ...
  float lon = planetLon(PLAN[sel], days);
  snprintf(buf, sizeof(buf), "%s  %3d  %.0fd",
           PLAN[sel].name, (int)lon, PLAN[sel].period);
  dStr(0, 63, buf);
}
...
static void nextFace() {
  ...
  faceFlushAt = millis() + 4000;       // debounced save, see facePersistTick
}
...
void setup() {
  ...
  bootLine("ORBITAL CLOCK " FW_VERSION, 420);
  bootLine("WAKE // COLD BOOT", 260);
  bootLine("SSD1309 PANEL .... OK", 240);
...
void loop() {
  ...
  switch (face) {
    ...
    case FACE_MOON:                     faceMoon();      break;
    case FACE_ORRERY:    starsDraw(H);  faceOrrery();    break;
